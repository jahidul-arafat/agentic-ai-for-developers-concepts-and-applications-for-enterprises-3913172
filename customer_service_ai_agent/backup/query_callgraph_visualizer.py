#!/usr/bin/env python3
"""
FIXED Query Execution Call Graph Visualizer
Fixed issues:
- Replaced non-existent updateCluster with proper node updates
- Fixed HTML tooltip rendering
- Proper vis.js API usage
"""
...
class EnhancedQueryExecutionTracker:
    ...
    def _generate_html_graph(self) -> str:
        """Generate interactive HTML graph with fixed tooltips and proper vis.js usage"""
    ...
    def _create_html_template(self, graph_data: Dict) -> str:
        """Create the complete HTML template with fixed JavaScript"""
        return f"""
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="header">
        <h1>🎯 Query Execution Call Graph - Fixed</h1>
        <div class="metadata">
...
Most Common Node Type: ${{stats.mostCommonType}}`;
        ...
        function showNodeDetails(node) {{
            ...
            // Find the original node data for more details
            const originalNode = originalNodeData.find(n => n.id === node.id);
            
            let cleanDetails = '<div class="node-details">';
            cleanDetails += `<h4>${{node.label}}</h4>`;
            cleanDetails += `<p><strong>Node ID:</strong> ${{node.id}}</p>`;
            cleanDetails += `<p><strong>Type:</strong> ${{node.shape}}</p>`;
            cleanDetails += `<p><strong>Color:</strong> ${{node.color.background}}</p>`;
            
            // Parse tooltip for additional info
            if (node.title) {{
                const tooltipLines = node.title.split('<br/>');
                cleanDetails += '<div style="margin-top: 10px; padding: 8px; background: #f0f0f0; border-radius: 4px;">';
                tooltipLines.forEach(line => {{
                    if (line.trim()) {{
                        cleanDetails += `<p style="margin: 2px 0; font-size: 13px;">${{line}}</p>`;
                    }}
                }});
                cleanDetails += '</div>';
            }}
            ...
            cleanDetails += '</div>';
            infoDiv.innerHTML = cleanDetails;
        }}
...
if __name__ == "__main__":
    print("🔧 FIXED Query Call Graph Visualizer")
    print("✅ Fixed Issues:")
    print("   - Replaced updateCluster with proper setData method")
    print("   - Fixed HTML tooltip rendering")
    print("   - Added proper error handling")
    print("   - Added keyboard shortcuts (Ctrl+F, Ctrl+H, Ctrl+R)")
    print("   - Improved node highlighting logic")
    print("=" * 50)