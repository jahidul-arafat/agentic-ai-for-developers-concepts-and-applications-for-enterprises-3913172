...
from live_call_graph_tracker import (
    call_graph_tracker,
    track_agent_execution,
    track_tool_execution,
    track_subgoal_execution,
    CallGraphAgentWrapper,
    run_query_with_live_visualization
)
...
class CustomerServiceTools:
    ...
    def get_item_return_days(self, item: str) -> int:
        ...
        return results[0][0] if results else 45
    ...
    def get_order_details(self, order_id: int) -> Dict[str, Any]:
        """Get comprehensive order details with call graph tracking"""
    ...
    def search_orders_by_customer(self, email: str) -> List[Dict]:
        """Search orders by customer email with call graph tracking"""
        query = """
    ...
    def analyze_customer_orders_comprehensive(self, customer_email: str) -> Dict[str, Any]:
        """Comprehensive analysis with call graph tracking"""
        query = """
                SELECT o.order_id,
                       o.order_date,
                       o.delivery_date,
...
class QueryDecomposer:
    ...
    def execute_decomposed_query(self, query: str) -> str:
        """Execute query with automatic decomposition and call graph tracking"""
        print(f"🔍 Analyzing query complexity...")

        # Add complexity analysis to call graph
        analysis_thought = call_graph_tracker.add_thought(
        ...
        if not assessment['requires_decomposition']:
            ...
            decision_id = call_graph_tracker.add_decision(
                "Query complexity assessment completed",
                options=["Standard execution", "Intelligent decomposition"],
                chosen="Standard execution",
                parent_id=analysis_thought
            )
        ...
        decision_id = call_graph_tracker.add_decision(
            f"Complex query detected (score: {assessment['complexity_score']})",
            options=["Standard execution", "Intelligent decomposition"],
        ...
        # Add subgoal nodes to call graph
        subgoal_ids = []
        for i, subgoal in enumerate(subgoals, 1):
            ...
            subgoal_ids.append((subgoal_id, subgoal))

        # Execute subgoals sequentially with call graph tracking
        results = []
        context = []

        for i, (subgoal_id, subgoal) in enumerate(subgoal_ids, 1):
            print(f"\n🎯 Executing subgoal {i}: {subgoal['subgoal']}")
            print("-" * 40)

            # Update subgoal status
            call_graph_tracker.update_subgoal_status(subgoal_id, "executing")
            ...
            # Add thought for subgoal execution
            execution_thought = call_graph_tracker.add_thought(
                f"Executing subgoal {i}: {subgoal['subgoal']}",
            ...
            try:
                ...
                # Update subgoal as completed
                call_graph_tracker.update_subgoal_status(subgoal_id, "completed", str(result)[:200])

                # Add successful observation
                call_graph_tracker.add_observation(
                    f"Subgoal {i} completed successfully",
                    parent_id=execution_thought,
                    success=True
                )
                ...
                # Update subgoal as failed
                call_graph_tracker.update_subgoal_status(subgoal_id, "failed", str(e))

                # Add failed observation
                call_graph_tracker.add_observation(
                    f"Subgoal {i} failed: {str(e)}",
                    parent_id=execution_thought,
                    success=False
                )
        ...
        # Synthesize final response with call graph tracking
        synthesis_thought = call_graph_tracker.add_thought(
        ...
        call_graph_tracker.add_observation(
            "Query decomposition and synthesis completed",
            parent_id=synthesis_thought,
            success=True
        )
...
class CustomerServiceAgent:
    ...
    def create_agent(self):
        """Create the enhanced AI agent with call graph tracking"""
        if not LLAMAINDEX_AVAILABLE or not self.tools:
            return False

        print("🤖 Creating enhanced AI agent with call graph tracking...")
        logger.info("Creating AI agent with call graph tracking enabled")
        ...
        try:
            ...
            agent_worker = ReActAgentWorker.from_tools(
                ...
                allow_parallel_tool_calls=config.agent_allow_parallel_tool_calls
            )

            # Create agent runner
            original_agent = AgentRunner(
                agent_worker,
                memory=None if not config.agent_memory_enabled else "buffer",
                verbose=config.agent_verbose
            )

            # Wrap with call graph tracking
            self.agent = CallGraphAgentWrapper(original_agent)

            print("✅ Enhanced AI agent created successfully!")
            print(f"   🔧 Max iterations: {config.max_iterations}")
            print(f"   🧠 Verbose mode: {'Enabled' if config.agent_verbose else 'Disabled'}")
            print(f"   📊 Call graph tracking: Enabled")
            print(f"   ⚡ Parallel tools: {'Enabled' if config.agent_allow_parallel_tool_calls else 'Disabled'}")

            logger.info(f"AI agent created with call graph tracking enabled")
            return True
    ...
    def run_custom_query(self):
        """Enhanced custom query with call graph visualization"""
        if not self.agent:
            print("❌ Agent not initialized!")
            return
        ...
        print("- Order details, delivery dates, return policies")
        print("- Customer support information")
        print("- Multi-order analysis, geographic analysis")
        print("- Predictive analysis, any combination of the above")
        print("\nSpecial commands:")
        print("- 'demo cache' - Demonstrate cache behavior")
        print("- 'demo callgraph' - Demonstrate call graph tracking")
        ...
        while True:
            ...
            if query.lower() == 'back':
            ...
            elif query.lower() == 'demo callgraph':
                self.demonstrate_call_graph_tracking()
            elif query:
                # Enhanced execution options with call graph
                print(f"\n📋 Execution options for: '{query[:60]}{'...' if len(query) > 60 else ''}'")
                print("1. 🚀 Standard execution")
                print("2. 📊 Enhanced tracking (shows detailed tool usage)")
                print("3. 🧩 Intelligent decomposition")
                print("4. 🎬 Live call graph visualization")  # NEW OPTION

                exec_choice = input("Select execution method (1-4, default=4): ").strip() or "4"
                ...
                if exec_choice == "1":
                    ...
                    if hasattr(self, 'query_decomposer') and self.query_decomposer:
                        ...
                        print("⚠️  Query decomposer not available")
                        self.run_query_with_enhanced_tracking(query)
                elif exec_choice == "4":
                    # NEW: Live call graph visualization
                    run_query_with_live_visualization(self, query)
    ...
    def run_query(self, query: str):
        """Execute a query using the agent with enhanced call graph tracking"""
        if not self.agent:
            print("❌ Agent not initialized!")
            return
        ...
        try:
            ...
            # Add timeout and retry logic with call graph tracking
            import signal
            ...
            try:
                # Add thought to call graph
                thought_id = call_graph_tracker.add_thought(
                    f"Executing query with agent: {query}"
                )
            ...
            call_graph_tracker.add_observation(
                f"Critical error during query execution: {e}",
                success=False
            )
    ...
    # Update the main menu to include call graph options
    def main_menu(self):
        """Main interactive menu with call graph tracking"""
        ...
        while True:
            print("\n" + "=" * 60)
            print("🤖 CUSTOMER SERVICE AI AGENT - WITH LIVE CALL GRAPH")
            print("=" * 60)
            ...
            print("10. 📊 Live Call Graph Report")  # NEW
            print("11. 📁 Export Call Graph")       # NEW
            print("12. 🎬 Demo Call Graph")         # NEW
            print("13. 🚪 Exit")
            print("=" * 60)
            ...
            if choice == '1':
                ...
                self.show_live_call_graph()      # NEW
            elif choice == '11':
                self.export_call_graph_report()  # NEW
            elif choice == '12':
                self.demonstrate_call_graph_tracking()  # NEW
            elif choice == '13':
                print("\n👋 Goodbye!")
                break
            else:
    ...
    def run_intelligent_query(self, query: str):
        """Enhanced query execution with automatic decomposition and call graph tracking"""
        ...
        try:
            ...
            # Add complexity analysis to call graph
            thought_id = call_graph_tracker.add_thought(
                "Analyzing query complexity for decomposition"
            )

            result = self.query_decomposer.execute_decomposed_query(query)

            call_graph_tracker.add_observation(
                "Query decomposition and execution completed",
                parent_id=thought_id,
                success=True
            )
            ...
            call_graph_tracker.add_observation(
                f"Intelligent query execution failed: {e}",
                success=False
            )
            print(f"❌ Intelligent query execution failed: {e}")

    def show_live_call_graph(self):
        """Display live call graph for current session"""
        if not call_graph_tracker.current_session:
            print("❌ No active call graph session")
            return

        report = call_graph_tracker.generate_call_graph_report()

        print("\n📊 LIVE CALL GRAPH REPORT")
        print("=" * 60)

        # Session info
        session_info = report['session_info']
        print(f"🆔 Session: {session_info['session_id']}")
        print(f"📝 Query: {session_info['query']}")
        print(f"⏱️  Duration: {session_info.get('duration', 0):.2f}s")
        print(f"🧩 Decomposed: {session_info['is_decomposed']}")
        ...
        print(f"\n🛣️  Recent Execution Path:")
        for i, step in enumerate(report['execution_path'][-10:], 1):
            icon = "🤔" if step['type'] == 'thought' else "⚡" if step['type'] == 'action' else "👁️"
            print(f"   {i}. {icon} {step['type'].title()}: {step['content'][:60]}...")
        ...
        if report['subgoals']:
            ...
            for i, subgoal in enumerate(report['subgoals'], 1):
                ...
                print(f"   {i}. {status_icon} {subgoal['content'][:60]}...")

    def export_call_graph_report(self):
        """Export current call graph to file"""
        if not call_graph_tracker.current_session:
            print("❌ No active call graph session to export")
            return

        print("\n📁 Export Call Graph Report")
        print("=" * 30)
        print("1. JSON format")
        ...
        if choice == "1":
            result = call_graph_tracker.export_call_graph(format="json")
        elif choice == "2":
            result = call_graph_tracker.export_call_graph(format="md")
        else:
    ...
    def demonstrate_call_graph_tracking(self):
        """Demonstrate call graph tracking with a sample query"""
        print("\n🎬 Call Graph Tracking Demonstration")
        print("=" * 50)

        # Sample queries to demonstrate different complexity levels
        sample_queries = [
            {
                "query": "What items are in order 1001?",
                "description": "Simple query - single tool call",
                "complexity": "Basic"
            },
            {
                "query": "Get order details for 1007, 1017, and 1023 and compare their delivery dates",
                "description": "Complex query - multiple tools, comparison logic",
                "complexity": "Advanced"
            },
            {
                "query": "Three customers have MacBook Pro issues - analyze their orders and recommend solutions",
                "description": "Expert query - decomposition, analysis, recommendations",
                "complexity": "Expert"
            }
        ]

        print("Available demonstration queries:")
        for i, sample in enumerate(sample_queries, 1):
            print(f"{i}. [{sample['complexity']}] {sample['query']}")
            print(f"   → {sample['description']}")
            print()

        choice = input("Select a query to demonstrate (1-3) or 'custom' for your own: ").strip()

        if choice in ['1', '2', '3']:
            selected_query = sample_queries[int(choice) - 1]
            query = selected_query['query']
            print(f"\n🎯 Demonstrating: {selected_query['description']}")
        ...
        print(f"\n🎬 Starting call graph demonstration...")
        print("👀 Watch how the agent breaks down and executes the query:")

        # Enable detailed live display for demonstration
        original_display = call_graph_tracker.live_display
        call_graph_tracker.live_display = True

        try:
            run_query_with_live_visualization(self, query)

            # Offer to export the call graph
            export_choice = input("\n📁 Export this call graph? (y/n): ").strip().lower()
            if export_choice == 'y':
                self.export_call_graph_report()

        finally:
            # Restore original display setting
            call_graph_tracker.live_display = original_display
